...
def save_checkpoint(cfg: CheckpointConfig, trainer, epoch_itr, val_loss):
    ...
    if cfg.distributed_rank == 0:
        PathManager.mkdirs(cfg.save_dir)
    ...
    checkpoints = [
        PathManager.join_path(cfg.save_dir, fn) for fn, cond in checkpoint_conds.items() if cond
    ]
    ...
    if not end_of_epoch and cfg.keep_interval_updates > 0:
        ...
        for old_chk in checkpoints[cfg.keep_interval_updates:]:
            if PathManager.lexists(old_chk):
...
def load_checkpoint(cfg: CheckpointConfig, trainer, **passthrough_args):
    ...
    *passthrough_args* will be passed through to
    ...
    if (
        ...
        checkpoint_path = PathManager.join_path(
...
def checkpoint_paths(path, pattern=r"checkpoint(\d+)\.pt"):
    ...
    return [PathManager.join_path(path, x[1]) for x in sorted(entries, reverse=True)]
...
def verify_checkpoint_directory(save_dir: str) -> None:
    ...
    temp_file_path = PathManager.join_path(save_dir, "dummy")
    try:
        with PathManager.open(temp_file_path, "w"):
            pass
    except OSError as e:
        logger.warning(
        ...
        if PathManager.exists(temp_file_path):
            PathManager.rm(temp_file_path)
