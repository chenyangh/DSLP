...
@register_model("nat_ctc_predlen")
class NATransformerModel(FairseqNATModel):
    ...
    def upsample_tokens(self, tokens):
        def _us(x, s):
            B = x.size(0)
            _x = x.unsqueeze(-1).expand(B, -1, s).reshape(B, -1)
            return _x
        return _us(tokens, self.args.src_upsample_scale)
    ...
    def forward(
        ...
        with torch.no_grad():
            prev_output_tokens = self.initialize_output_tokens_by_trg_tokens(encoder_out, src_tokens,
                                         length_tgt=torch.sum(tgt_tokens.ne(self.tgt_dict.pad_index), dim=-1))
    ...
    def initialize_output_tokens_by_trg_tokens(self, encoder_out, src_tokens, length_tgt=None):
    ...
    def initialize_output_tokens(self, encoder_out, src_tokens):
        ...
        initial_output_tokens = self.initialize_output_tokens_by_trg_tokens(encoder_out, src_tokens)
