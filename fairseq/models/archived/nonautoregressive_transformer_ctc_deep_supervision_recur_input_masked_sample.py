...
@register_model("nat_ctc_deep_supervision_recur_input_masked_sample")
class NATransformerModel(FairseqNATModel):
    ...
    def sequence_ctc_loss_with_logits(self,
        ...
        if label_smoothing > 0:
            ...
            kl_loss = F.kl_div(log_probs, torch.full_like(log_probs, 1 / n_vocob), reduction='none',
                               log_target=False).sum(-1)
            ...
            loss = (1 - label_smoothing) * loss + label_smoothing * kl_loss
        return loss
    ...
    def forward(
        ...
        num_decoder_layer = self.decoder.num_layers
        num_tokens = prev_output_tokens.size(1)
        num_vocab = len(self.tgt_dict)
        batch_size = prev_output_tokens.size(0)

        all_sample_ctc_loss = 0
        N_SAMPLE = self.args.num_cross_layer_sample
        for sample_id in range(N_SAMPLE):
            # Sample cross-layer logits
            cross_layer_sampled_ids_ts = torch.randint(num_decoder_layer, (batch_size * num_tokens,)).to(
                src_tokens.device)

            # get decoder output with layer wise masking
            output_logits_list = self.decoder(
                normalize=False,
                prev_output_tokens=prev_output_tokens,
                encoder_out=encoder_out,
                self_attn_mask=cross_layer_sampled_ids_ts
            )
    ...
    def forward_decoder(self, decoder_out, encoder_out, decoding_format=None, **kwargs):
        ...
        output_logits = output_logits_list[-1] # take the last layer
...
class NATransformerDecoder(FairseqNATDecoder):
    ...
    def forward(self, normalize, encoder_out, prev_output_tokens, step=0, **unused):
        self_attn_mask = unused.get('self_attn_mask', None)
        ...
        all_layer_out_list = all_features['layer_output']

        # decoder_out = self.output_layer(features)
        return [F.log_softmax(x, -1) if normalize else x for x in all_layer_out_list]
    ...
    def extract_features(
        ...
        **unused
        ...
        all_layer_mask_id = unused.get('self_attn_mask', None)
        # decoder layers
        for i, layer in enumerate(self.layers):
            # Note: needs to make layer-wise mask
            per_layer_mask = None
            if all_layer_mask_id is not None:
                num_decoder_heads = self.args.decoder_attention_heads
                if i >= 0:
                    per_layer_mask = torch.full((x.size(1) * num_decoder_heads, x.size(0), x.size(0)), 0.).to(x)
                else:
                    per_layer_mask = torch.full((x.size(1), x.size(0)), 0.).to(x)
                    per_layer_mask.masked_fill_(all_layer_mask_id.view(x.size(1), x.size(0)) != i-1, -10.) # float("-inf")
                    per_layer_mask[torch.all(torch.isinf(per_layer_mask), dim=1)] = 0.
                    # Note: current version ignores the case of disconnected layer, for simplicity in implementation
                    per_layer_mask = per_layer_mask.unsqueeze(1).repeat(1, per_layer_mask.size(1), 1)
                    per_layer_mask = per_layer_mask.repeat_interleave(num_decoder_heads, 0)  # Note this could be wrong
            ...
            x, attn, _ = layer(
                ...
                self_attn_mask=per_layer_mask,
                self_attn_padding_mask=decoder_padding_mask,
            )
            inner_states.append(x)
            layer_out_logits = self.output_layer(x)
            temperature = 0.2 # NOTE
            layer_out = torch.matmul(torch.softmax(layer_out_logits.detach()/temperature, dim=-1).view(-1, layer_out_logits.size(-1)), self.embed_tokens.weight).view(x.size())
        ...
        return x, {"attn": attn, "inner_states": inner_states, "layer_output": layer_output_list}
...
@register_model_architecture(
    "nat_ctc_deep_supervision_recur_input_masked_sample", "nat_ctc_deep_supervision_recur_input_masked_sample"
)
