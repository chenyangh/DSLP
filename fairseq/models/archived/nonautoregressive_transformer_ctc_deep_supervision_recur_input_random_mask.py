...
@register_model("nat_ctc_deep_supervision_recur_input_random_mask")
class NATransformerModel(FairseqNATModel):
    ...
    def forward(
        ...
        cross_layer_sampled_ids_ts = True  # NOTE: as long as it is not 'None'
        ...
        for output_logits in output_logits_list:
...
class NATransformerDecoder(FairseqNATDecoder):
    ...
    def extract_features(
        ...
        **unused
        ...
        mask_rate = self.args.random_mask_rate
        all_layer_mask_id = unused.get('self_attn_mask', None)  # Note: not used, just to keep the interface compatible ftm.
        # decoder layers
        for i, layer in enumerate(self.layers):
            ...
            if all_layer_mask_id is not None:
                ...
                if i == 0:
                    per_layer_mask = torch.full((x.size(1) * num_decoder_heads, x.size(0), x.size(0)), 0., device=x.device)
                else:
                    per_layer_mask = torch.full((x.size(1) * num_decoder_heads, x.size(0), x.size(0)), mask_rate, device=x.device) # totally random mask
                    per_layer_mask = torch.bernoulli(per_layer_mask)
                    per_layer_mask[per_layer_mask==1] = -1e5
            ...
            # new_x = x + layer_out
            x, attn, _ = layer(
                x,  # new_x Note: change it back for now
...
@register_model_architecture(
    "nat_ctc_deep_supervision_recur_input_random_mask", "nat_ctc_deep_supervision_recur_input_random_mask"
)
