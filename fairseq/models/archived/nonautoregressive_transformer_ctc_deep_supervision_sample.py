...
@register_model("nat_ctc_deep_supervision_sample")
class NATransformerModel(FairseqNATModel):
    ...
    def forward(
        ...
        for sample_id in range(N_SAMPLE):
            ...
            cross_layer_sampled_ids_ts = torch.randint(num_decoder_layer, (batch_size * num_tokens, )).to(output_logits_list.device)
            ...
            # # cross_layer_sampled_ids_ts = torch.randint(num_decoder_layer, (batch_size * num_tokens,)).to(output_logits_list.device)
            # # output_logits_list = output_logits_list.view(num_decoder_layer, -1, num_vocab)
            # gather_idx2 = cross_layer_sampled_ids_ts.unsqueeze(1).repeat(1, num_vocab).unsqueeze(0)
            # gather_logits2 = output_logits_list.gather(0, gather_idx2).view(batch_size, num_tokens, num_vocab)
            #
            # manual_gather = torch.zeros(batch_size * num_tokens, num_vocab).cuda()
            # for _idx, _layer in enumerate(cross_layer_sampled_ids_ts.tolist()):
            #     manual_gather[_idx] = output_logits_list[_layer][_idx]
            #
            # gather_logits3 = manual_gather.view(batch_size, num_tokens, num_vocab)
            # # time_all += time.time() - start

            # assert torch.all(gather_logits == gather_logits2)
...
class NATransformerDecoder(FairseqNATDecoder):
    ...
    def forward(self, normalize, encoder_out, prev_output_tokens, step=0, **unused):
        ...
        all_layer_out_list = []
        for layer_out in all_features['inner_states'][1:]:
            all_layer_out_list.append(self.output_layer(layer_out.transpose(0, 1)))
...
@register_model_architecture(
    "nat_ctc_deep_supervision_sample", "nat_ctc_deep_supervision_sample"
)
