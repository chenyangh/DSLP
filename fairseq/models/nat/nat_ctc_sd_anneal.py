...
import logging
import random, math
from fairseq.models.nat.levenshtein_utils import load_libnat
libnat, _ = load_libnat()
...
@register_model("nat_ctc_sd_anneal")
class NATransformerModel(FairseqNATModel):
    ...
    def forward(
        ...
        cur_device_id = prev_output_tokens.device
        ...
        all_layer_lev_dist = []
        if self.training:
            # Step 1: get per layer lev distance
            ctc_beam_size_for_restore = self.ctc_decoder._beam_width
            self.ctc_decoder._beam_width = 1
            self.eval()
            with torch.no_grad():
                ...
                for per_layer_output_logits in output_logits_list:
                    output_length = torch.sum(prev_output_tokens.ne(self.tgt_dict.pad_index), dim=-1)
                    beam_results, beam_scores, timesteps, out_lens = self.ctc_decoder.decode(F.softmax(per_layer_output_logits, -1),
                                                                                             output_length)
                    top_beam_tokens = beam_results[:, 0, :].to(cur_device_id)
                    top_beam_len = out_lens[:, 0].to(cur_device_id)
                    pred_mask = torch.arange(0, top_beam_tokens.size(1), device=cur_device_id).to(top_beam_len.dtype). \
                        repeat(top_beam_len.size(0), 1).lt(top_beam_len.unsqueeze(1))
                    top_beam_tokens[~pred_mask] = self.decoder.dictionary.pad()
                    # Note: compute distance with GT
                    ops = libnat.levenshtein_distance(
                                tgt_tokens.int(),
                                top_beam_tokens.int(),
                    ...
                    ops[ops==3] = 0
                    ops[ops!=0] = 1
                    dist = 1 - torch.div(torch.sum(ops, dim=-1), target_mask.sum(1) + pred_mask.sum(1))
                    # Note: annealed keep ratio has to be on sentence level.(fine-grained)
                    all_layer_lev_dist.append(dist)
            self.ctc_decoder._beam_width = ctc_beam_size_for_restore
        ...
        **unused
        ...
        for i, layer in enumerate(self.layers):
            ...
            if feed_forward_weights is None:
                new_x = x  # (x + layer_out) / torch.sqrt(torch.tensor(2.))  # affect how to decode
            else:
                a, b = feed_forward_weights[i]
                a, b = a*1.0, b*1.0  # force float type
                new_x = (a * x + b * layer_out) / torch.sqrt(torch.tensor(torch.square(a) + torch.square(b)))
            x, attn, _ = layer(
                new_x,
...
@register_model_architecture(
    "nat_ctc_sd_anneal", "nat_ctc_sd_anneal"
)
