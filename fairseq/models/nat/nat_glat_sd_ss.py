...
class NATransformerModel(FairseqNATModel):
    ...
    def forward(
        ...
        glat_info = None
        anneal_info = None
        keep_word_mask_list = None
        glat_tgt_tokens_list = None
        given_tgt_tokens = None
        if glat and tgt_tokens is not None:
            if "context_p" in glat:
                with torch.no_grad():
                    ...
                    for per_layer_word_ins in word_ins_out_list:
                        pred_tokens = per_layer_word_ins.argmax(-1)
                    ...
                    keep_word_mask_list = []
                    if not self.args.only_depend_on_last:
                        keep_word_mask_0 = (torch.rand(prev_output_tokens.shape,
                                                    device=word_ins_out_list[-1].device) < keep_prob_list[0]).bool()
                        keep_word_mask_list.append(keep_word_mask_0)
                        for _idx in range(len(keep_prob_list)):
                            if _idx == 0:
                                continue
                            further_keep_ratio = keep_prob_list[_idx] / keep_prob_list[_idx-1]
                            next_keep_mask = ((torch.rand(prev_output_tokens.shape, device=word_ins_out_list[-1].device) < further_keep_ratio)  
                                                & keep_word_mask_list[_idx-1]).bool()
                            keep_word_mask_list.append(next_keep_mask)
                    else:
                        keep_word_mask_last = (torch.rand(prev_output_tokens.shape,
                                                    device=word_ins_out_list[-1].device) < keep_prob_list[-1]).bool()

                        keep_word_mask_list = [keep_word_mask_last for _ in range(len(keep_prob_list))]

                    glat_tgt_tokens_list = []
                    for _keep_word_mask in keep_word_mask_list:
                        if self.args.only_mask_first:
                            glat_tgt_tokens = tgt_tokens.masked_fill(keep_word_mask_list[0], self.pad)
                        else:
                            glat_tgt_tokens = tgt_tokens.masked_fill(_keep_word_mask, self.pad)
                        glat_tgt_tokens_list.append(glat_tgt_tokens)

                    prev_output_tokens = prev_output_tokens.masked_fill(keep_word_mask_list[0], 0) + tgt_tokens.masked_fill(~keep_word_mask_list[0], 0)
                    given_tgt_tokens = tgt_tokens
                    
                    glat_info = {
                        # "glat_accu": (same_num.sum() / seq_lens.sum()).item(),
                        "glat_context_p": glat['context_p'],
                        # "glat_keep": keep_prob_list[-1].mean().item()
                    }
                    for _idx, keep_prob in enumerate(keep_prob_list):
                        glat_info.update({f"glat_keep_{_idx}": keep_prob.mean().item()})
        ...
        with torch_seed(rand_seed):
            word_ins_out_list = self.decoder(
                ...
                prev_output_tokens=prev_output_tokens,
                encoder_out=encoder_out,
                keep_word_mask_list=keep_word_mask_list,
                given_tgt_tokens=given_tgt_tokens
            )
        ...
        if glat_tgt_tokens_list is None:
            glat_tgt_tokens_list = [tgt_tokens for _ in range(len(word_ins_out_list))]

        for _idx, (_word_ins_out, _tgt_tokens) in enumerate(zip(word_ins_out_list, glat_tgt_tokens_list)):
...
class NATransformerDecoder(FairseqNATSharedDecoder):
    ...
    @ensemble_decoder
    def forward(self, normalize, encoder_out, prev_output_tokens, step=0, train_ratio=None, keep_word_mask_list=None, given_tgt_tokens=None, **unused):
        ...
        for i, layer in enumerate(self.layers):
            ...
            if  i == 0:
                ...
                if self.args.sample_option == 'hard':
                    ...
                    if keep_word_mask_list is not None:
                        fused_tokens = y_hat_tokens * ~keep_word_mask_list[i].transpose(0, 1) + given_tgt_tokens.transpose(0, 1) * keep_word_mask_list[i].transpose(0, 1)
                    else:
