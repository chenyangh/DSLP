...
from fairseq.models.nat.nat_sd_shared import _gumbel_softmax
import os
...
class NATransformerModel(FairseqNATModel):
    def __init__(self, args, encoder, decoder):
        ...
        self.num_decoder = getattr(args, 'num_decoder', 5)
        self.tie_decoder = getattr(args, 'tie_decoder', False)
        self.encoder_out_map = torch.nn.Linear(self.args.encoder_embed_dim, self.num_decoder*self.args.encoder_embed_dim)
        self.map_prob_predictor = torch.nn.Linear(self.args.encoder_embed_dim, self.num_decoder)
        self.model_tb = _get_summary_writer()(os.path.join(self.args.tensorboard_logdir, 'para'))
    ...
    @staticmethod
    def _meam_pool_encoder_out(_encoder_out, _sg=False):
        enc_feats = _encoder_out["encoder_out"][0]  # T x B x C
        if len(_encoder_out["encoder_padding_mask"]) > 0:
            src_masks = _encoder_out["encoder_padding_mask"][0]  # B x T
        else:
            src_masks = None
    ...
    def forward(
        ...
        # decoder_out = self.encoder(tgt_tokens, src_lengths=torch.sum(tgt_tokens.ne(self.pad), dim=-1))

        # bsz = src_tokens.size(0)
        encoder_mean_pool = self._meam_pool_encoder_out(encoder_out)
        # decoder_mean_pool = self._meam_pool_encoder_out(decoder_out)
        # encoder_map = self.encoder_out_map(encoder_mean_pool)
        map_dist_logits = self.map_prob_predictor(encoder_mean_pool)
        encoder_map_entropy = (map_dist_logits.softmax(-1) * map_dist_logits.log_softmax(-1)).sum(-1).mean()

        gumbel_sample = _gumbel_softmax(map_dist_logits, hard=not getattr(self.args, "gumbel_sm", False))

        encoder_out_size = encoder_out['encoder_out'][0].size()
        sampled_encoder_out = torch.matmul(self.encoder_out_map(encoder_out['encoder_out'][0]).reshape(*encoder_out_size[:3], self.num_decoder).transpose(0, 1), gumbel_sample.unsqueeze(1).unsqueeze(3))

        encoder_out['encoder_out'][0] = sampled_encoder_out.squeeze(3).transpose(0, 1)
        ...
        return {
            ...
            "map_entropy": {
                "loss": encoder_map_entropy,
                "factor": 0.1,
            },
            "stat:latent_dist": {
                "latent_dist": map_dist_logits.argmax(-1),
                "factor": 0.0,
            },
        }
    ...
    def forward_encoder(self, encoder_inputs):
        ...
        gumbel_sample = _gumbel_softmax(map_dist_logits, hard=not self.args.gumbel_sm)

        encoder_out_size = encoder_out['encoder_out'][0].size()
        sampled_encoder_out = torch.matmul(
            self.encoder_out_map(encoder_out['encoder_out'][0]).reshape(*encoder_out_size[:3],self.num_decoder)
                .transpose(0, 1), gumbel_sample.unsqueeze(1).unsqueeze(3))
