...
class MultiprocessingEncoder(object):
    def __init__(self, tgt_dict, tokenizer):
        ...
        self.pad_index = tgt_dict.pad_index
    ...
    def decode(self, tokens):
        # global bpe
        # tokens = tokens[tokens!=self.pad_index]
        tokens = [x for x in tokens if x != self.pad_index]
...
@register_task("translation_imitation_with_ranked_generator")
class TranslationImitationTaskWithRankedGenerator(TranslationTask):
    """
    ...
    def build_generator(self, models, args, **unused):
        ...
        from fairseq.iterative_refinement_generator import IterativeImitationRefinementGeneratorRankedByBleu

        generator = IterativeImitationRefinementGeneratorRankedByBleu(
        ...
        generator.bpe_deocde = MultiprocessingEncoder(self.tgt_dict, self.tokenizer)
        return generator
    ...
    def train_step(
        ...
        if update_num < starting_step:
            gt_sample_loss, sample_size, logging_output = criterion(model, sample)
            optimizer.backward(gt_sample_loss)
            return gt_sample_loss, sample_size, logging_output
        else:
            ...
            with torch.no_grad():
                ...
                search_results_bleu = self.get_per_sent_bleu_batch_by_beam(sample, search_results)
            ...
            cur_ratio = min(max_ratio, (update_num - starting_step) / (max_step - starting_step) * max_ratio)

            combined_loss = cur_ratio * positive_loss + (1-cur_ratio) * gt_sample_loss
