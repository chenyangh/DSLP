...
def last_n_checkpoints(paths, n, ckpt_based_on, upper_bound=None):
    assert len(paths) == 1
    path = paths[0]
    if ckpt_based_on == 'update':
        pt_regexp = re.compile(r"checkpoint_\d+_(\d+)\.pt")
    elif ckpt_based_on == 'epoch':
        pt_regexp = re.compile(r"checkpoint(\d+)\.pt")
    elif ckpt_based_on == 'best':
